...
export type PromotionDecision = 'Naik Kelas' | 'Tinggal di Kelas' | string;
...
export interface RaportSubject {
  ...
  /**
   * Backend `/raport/initial-data` ships these recap fields per subject
   * (Flutter consumes them for the Nilai tab preview). They mirror the
   * UH average + UTS + UAS columns so the teacher doesn't have to
   * cross-check the grade book.
   */
  recap_uh_avg?: number | null;
  recap_uts?: number | null;
  recap_uas?: number | null;
  recap_final_score?: number | null;
  recap_bab_scores?: number[];
  recap_bab_names?: string[];
}

export interface RaportExtra {
  id?: string;
  /** Name of the ekstrakurikuler. */
  name: string;
  /** Numeric score or descriptive label ("A", "Sangat Baik"). */
  score?: string;
  description?: string;
}

export interface RaportAchievement {
  id?: string;
  name: string;
...
/** Summary block injected on `/raport/show`. */
...
export interface RaportInitialData {
  student_class_id: string;
  student_id?: string;
  student_name?: string;
  class_name?: string;
...
/**
 * Per-class row returned by `/raports/teacher-summary`. Drives the
 * Frame A class-hub cards (4-cell stats + progress ring + drill).
 */
export interface RaportClassSummary {
  class_id: string;
  class_name: string;
  grade_level?: string | number | null;
  student_count: number;
  total_raports: number;
  draft_count: number;
  final_count: number;
...
export interface RaportSummaryRow {
  student_class_id: string;
  student_id?: string;
  student_name: string;
  ...
  raport_status?: ReportCardStatus | null;
  avg_grade?: number | null;
  remed_count?: number;
  published_at?: string | null;
}
...
export interface AdminRaportPeriod {
...
export interface AdminRaportPipeline {
  pipeline: PipelineNode[];
  tingkats: TingkatGroup[];
  period: AdminRaportPeriod;
  total_raports: number;
...
/**
 * Row returned by `GET /parent/raports` (one per child with a
 * published raport for the active TP+semester).
 */
export interface ParentRaportRow {
  student_class_id: string;
