<!--
  Tutoring2StudentScores.vue — ONE student's graded-score history,
  rendered the same way for every caller allowed to read it.

  ── Why this component exists ──

  The rendering started life inside `ParentTutoring2ProgressView.vue`,
  where the wali was its only reader. When the staff Peringkat boards
  gained a tappable row (Luay, 2026-09 — "Pada website belum menampilkan
  detail nilai siswa") a SECOND and THIRD caller needed the same view for
  a student who is not the reader's child. Writing a staff-side scores
  view would have created two renderings of the same payload that drift
  apart — and the honest-fallback rules below are exactly the kind of
  thing that drifts. So the body was extracted here and the wali view
  became a thin wrapper over it.

  This mirrors what the Flutter app did with
  `bimbel_student_scores_screen.dart`, deliberately: the same payload
  should not read differently on web and on mobile either.

  ── Endpoint ──

  `GET /tutoring-v2/students/{id}/progress`, via
  `TutoringStudentsService.getProgress`. ONE call. It replaced a
  client-side rebuild that fetched enrollments, then every assessment,
  then one scores call PER ASSESSMENT, capped at 40 to stop it running
  away.

  Everything the server sends is final. In particular:
    • `points` arrive oldest-first and already exclude drafts. Do not
      re-sort or re-filter for those reasons. The draft filter is what
      makes the endpoint safe to expose to a wali at all — do not add a
      client-side draft toggle.
  ...
  `StudentProgressController::authorizeStudentRead` recognises three
  readers and narrows each one:

    • an admin (`tutoring.score.view` AND the admin-only discriminator
      `tutoring.tutor.view`) reads any student on the tenant;
    • a tutor (`tutoring.score.view` alone) is narrowed to students
      enrolled in the groups they teach — anyone else is a 403, not an
      empty 200;
    • a wali or siswa (`tutoring.score.view_own`) is narrowed to the
      students they own.

  This component therefore does NOT re-derive who you are. Each caller
  gates its own entry point on the ability it holds, and the server
  re-checks regardless. What this file owns is being honest about what
  came back.
  ...
  Also gone for good, from the legacy wali view: the STUB_CHILD /
  STUB_AVG hardcoded arrays ([70,73,75,…]) that drew a fake ascending
  line whenever the API returned nothing. That was invented data
  presented as a real child's progress. An honest "not enough graded
  assessments yet" note takes its place.

  ── Why the header is a SLOT ──

  The three callers name the subject differently and cannot share a
  title: a wali reads "Perkembangan nilai" about their own child, while
  staff arriving from a board of many students need that student's NAME
  as the title to know whose marks they are reading. The slot receives
  `{ count, loading }` so each caller can still render the same meta line
...
const props = defineProps<{
  /** Whose scores to read. Empty renders the empty state, never a request. */
  studentId: string;
  /**
   * What to call the plotted series in the chart legend.
   *
   * The one string this component cannot own: a wali reads "Nilai anak"
   * about their own child; staff read "Nilai siswa" about someone
   * else's. Everything else is subject-neutral and lives in
   * `tutoring2.studentScores.*`.
   */
  legendSubjectLabel: string;
}>();

const { t } = useI18n();

/** One graded assessment for this student. */
...
// content branch over nothing instead of the empty state.
const { state, reload } = useDataRefresh<ProgressPayload | null>(async () => {
  ...
  const progress = await TutoringStudentsService.getProgress(props.studentId);
...
const kpiCards = computed<KpiCard[]>(() => {
  ...
  return [
    {
      icon: 'chart-bar',
      label: t('tutoring2.studentScores.kpiAverage'),
    ...
    {
      icon: 'award',
      label: t('tutoring2.studentScores.kpiHighest'),
      value: formatPercent(highest),
      tone: 'green',
    },
    {
      icon: 'trending-down',
      label: t('tutoring2.studentScores.kpiLowest'),
      value: formatPercent(lowest),
      tone: 'amber',
    },
    {
      icon: 'clipboard-list',
      label: t('tutoring2.studentScores.kpiGraded'),
 ...
 * is the mean of the student's programs' means. Null when no leaderboard
...
/**
 * KKM-relative tone. `kkmPercent` is the assessment's own passing mark
 * ALREADY on the 0..100 scale, so this is a real pass/fail signal rather
 * than an invented grade band. With no KKM set the row stays neutral —
 * we do not guess a threshold.
 */
...
function scoreLabel(point: ScorePoint): string {
  if (point.kkmPercent === null) return t('tutoring2.studentScores.noKkm');
  return point.percent >= point.kkmPercent
    ? t('tutoring2.studentScores.passed')
    : t('tutoring2.studentScores.belowKkm');
}

// The header slot needs the same two facts the meta line is built from,
// so a caller never has to reach into this component's state.
defineExpose({ reload });
</script>
...
<template>
  <div class="space-y-md pb-24">
    ...
    <AsyncView
      ...
      :loading-rows="3"
      :empty-title="t('tutoring2.studentScores.emptyTitle')"
      :empty-description="t('tutoring2.studentScores.emptyDesc')"
      @retry="reload"
    >
      <template #default>
        <p
        ...
        >
          {{ t('tutoring2.studentScores.noGradedYet') }}
        </p>
        ...
        <template v-else>
          ...
          <section class="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm">
            <h2 class="text-sm font-bold text-slate-900">
              {{ t('tutoring2.studentScores.chartTitle') }}
            </h2>

            <p v-if="!canRenderChart" class="mt-3 text-2xs text-slate-500">
              {{ t('tutoring2.studentScores.chartNeedsMorePoints') }}
            </p>
            ...
            <template v-else>
              <svg
                ...
                role="img"
                :aria-label="t('tutoring2.studentScores.chartTitle')"
              >
              ...
              <div class="mt-2 flex flex-wrap justify-between gap-2 text-2xs text-slate-500">
                ...
                <span v-if="peerAverage !== null" class="inline-flex items-center gap-1.5">
                  ...
                  {{
                    t('tutoring2.studentScores.legendPeer', {
