<!--
  ...
  When `kamu=true` the row is highlighted with a cobalt tint + left
...
<template>
  ...
  >
    ...
    >
      ...
      />
      <span v-else>{{ initials(entry.nama) }}</span>
    </div>
    <!-- Name + streak -->
    <div class="min-w-0 flex-1">
      <p class="text-sm font-bold text-slate-900 truncate">
        {{ entry.nama }}<span v-if="entry.kamu" class="ml-1 text-2xs font-bold text-brand-cobalt">(kamu)</span>
      </p>
      <p class="text-3xs text-slate-500 mt-0.5 flex items-center gap-2">
        <span class="flex items-center gap-1">
          <NavIcon name="flame" :size="12" />
          {{ entry.hari_beruntun }} hari
        </span>
        <span>·</span>
        <span>L{{ entry.level }}</span>
        <span v-if="entry.jumlah_badge > 0">·</span>
        <span v-if="entry.jumlah_badge > 0" class="flex items-center gap-1">
    ...
    <p class="text-sm font-black text-slate-900 flex-shrink-0">
      {{ entry.poin }}<span class="text-3xs text-slate-500 font-bold ml-1">XP</span>
    </p>
  </div>
</template>
