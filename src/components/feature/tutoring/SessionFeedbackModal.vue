...
<template>
  <Modal title="Beri Rating Sesi" @close="emit('close')">
    <p v-if="sessionTitle" class="text-xs text-slate-500 mb-3">
      {{ sessionTitle }}
    </p>
    <div class="flex items-center justify-center gap-1 my-3">
      <button
        ...
        :class="n <= rating ? 'text-amber-400' : 'text-slate-300 hover:text-amber-300'"
        @click="rating = n"
      >
        <NavIcon name="sparkles" :size="28" />
      </button>
    </div>
    <p class="text-center text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">
      {{ rating }} dari 5
    </p>
    ...
    <label class="block">
      ...
      <textarea
        ...
        class="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-role-parent/20 focus:border-role-parent resize-none"
      />
    </label>

    <p v-if="errMsg" class="text-xs text-status-danger mt-2">{{ errMsg }}</p>
