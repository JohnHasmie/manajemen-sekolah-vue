  ...
  >
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <p
          v-if="kicker"
          class="text-[10.5px] font-semibold uppercase tracking-widest text-white/75"
        >
          {{ kicker }}
        </p>
        ...
        <p v-if="subtitle" class="mt-1 text-[12.5px] text-white/85">{{ subtitle }}</p>
      </div>
    ...
    >
      ...
      >
        <p class="text-[9.5px] font-bold uppercase tracking-widest text-white/70">
          {{ s.label }}
        </p>
        <p class="mt-0.5 text-lg font-extrabold tracking-tight">{{ s.value }}</p>
        <p v-if="s.hint" class="text-[10.5px] text-white/65 truncate">{{ s.hint }}</p>
      </div>
    </div>
  </div>
</template>
