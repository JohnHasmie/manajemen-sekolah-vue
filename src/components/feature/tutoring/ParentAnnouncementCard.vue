...
<template>
  <article class="rounded-2xl border border-bimbel-border-soft bg-bimbel-panel p-3.5">
    <div class="mb-2 flex items-center gap-2">
      ...
      <span class="text-[11px] text-bimbel-text-mid">
        {{ sourceName }}<template v-if="context"> · {{ context }}</template>
        <template v-if="rel"> · {{ rel }}</template>
      </span>
      <span
        class="ml-auto rounded-full px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-wider"
        :class="chipClass"
      >
        {{ chipLabel }}
      </span>
    </div>
    <h3 class="text-[13.5px] font-extrabold tracking-tight text-bimbel-text-hi">
      {{ title }}
    </h3>
