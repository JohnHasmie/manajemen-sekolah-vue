...
<template>
  ...
  >
    <div class="w-full max-w-md rounded-2xl bg-bimbel-panel p-5 shadow-xl">
      <h3 class="text-[16px] font-bold text-bimbel-text-hi">{{ title }}</h3>
      <p class="mt-1 text-[13px] text-bimbel-text-mid whitespace-pre-wrap">{{ message }}</p>
      <div class="mt-4 flex gap-2">
        ...
        <button
          ...
          :class="danger ? 'bg-rose-600 hover:bg-rose-700' : 'bg-bimbel-accent hover:opacity-90'"
          @click="emit('confirm')"
