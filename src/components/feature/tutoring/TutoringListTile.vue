...
<template>
  <button
    type="button"
    class="w-full flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-2xl text-left transition hover:border-slate-300"
    @click="handle"
  >
    <span
    ...
    <span class="min-w-0 flex-1">
      <span class="block text-[13.5px] font-semibold text-slate-900 tracking-tight">{{ title }}</span>
      <span v-if="subtitle" class="block text-xs text-slate-500 mt-0.5">{{ subtitle }}</span>
    </span>
