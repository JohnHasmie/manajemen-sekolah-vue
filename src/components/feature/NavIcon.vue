<!--
  NavIcon.vue — lucide-style SVG icons for nav menu items.
  Keeps the bundle lean by hand-rolling the few we use instead of
  pulling in a full icon library.
-->
...
<template>
  ...
  >
    ...
    <template v-else-if="name === 'megaphone'">
      <polygon points="3 11 22 2 22 22 3 13" />
      <path d="M11 11v6" />
    </template>
    ...
    <template v-else-if="name === 'gauge'">
      <!-- lucide `gauge` — half-arc dial with needle. Used by the admin
           Pusat Kendali (readiness) nav entry to visually cue "kesiapan
           sekolah" without leaning on emoji. -->
      <path d="m12 14 4-4" />
