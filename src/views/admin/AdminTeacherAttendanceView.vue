...
const tab = ref<Tab>((route.query.tab as Tab) || (mode.value === 'settings' ? 'settings' : 'report'));
...
watch(tab, (newTab) => {
  if (newTab === 'rules' || newTab === 'checkin_rules') {
...
function getCheckinRuleDescription(rule: any): string {
  if (!rule.checkin_time_validation_enabled) return 'Validasi dinonaktifkan';
  if (rule.checkin_time_rule_type === 'all_days') {
    return `Maksimal: ${rule.checkin_time_all_days}`;
  }
  return 'Kustom per hari';
}
  ...
  <div class="space-y-md">
    ...
    >
      ...
      >
        ...
        >
          <NavIcon name="clock" :size="13" />{{ t('admin.sekolah.teacher_attendance.tab_checkin_rules') }}
        </button>
        <button
          v-if="mode === 'settings'"
          type="button"
