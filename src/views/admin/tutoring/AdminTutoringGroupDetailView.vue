  ...
  <div class="space-y-4 pb-12">
    ...
    >
      <template #actions>
        ...
        <button class="rounded-lg bg-white text-tutoring-accent px-3 py-1.5 text-[14px] font-bold"><NavIcon name="plus" :size="12" class="inline -mt-0.5" /> {{ t('admin.bimbel.group_detail.add_student') }}</button>
      </template>
    </TutorHomeHero>

    <div class="flex gap-1 rounded-xl border border-tutoring-border-soft bg-tutoring-panel p-1 max-w-md">
      <button
        ...
        :class="tab === tk ? 'bg-tutoring-accent text-white' : 'text-tutoring-text-mid hover:text-tutoring-text-hi'"
        @click="tab = tk"
      >
    ...
    <div v-if="loading" class="py-12 text-center text-tutoring-text-mid">{{ t('admin.bimbel.group_detail.loading') }}</div>

    <template v-else>
      <template v-if="tab === 'students'">
        <div v-if="enrollees.length === 0" class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-8 text-center text-sm text-tutoring-text-mid">
          {{ t('admin.bimbel.group_detail.empty_students') }}
        </div>
        <div v-else class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel overflow-hidden">
          <table class="w-full text-[14px]">
            ...
            <tbody>
              <tr v-for="e in enrollees" :key="e.id" class="border-t border-tutoring-border-soft">
                <td class="px-3 py-2.5 font-bold text-tutoring-text-hi">{{ e.student?.name ?? '—' }}</td>
      ...
      <template v-else-if="tab === 'sessions'">
        <div v-if="sessions.length === 0" class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-8 text-center text-sm text-tutoring-text-mid">
          {{ t('admin.bimbel.group_detail.empty_sessions') }}
        </div>
        <div v-else class="space-y-2">
          <div v-for="s in sessions" :key="s.id" class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-3">
            <div class="flex items-center justify-between text-[13px] text-tutoring-text-mid">
              <span>{{ whenLabel(s.scheduled_at) }} · {{ s.duration_minutes }}m</span>
              <span class="rounded-full px-2 py-0.5 text-[13px] font-bold" :class="s.status === 'DONE' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-tutoring-accent-dim text-tutoring-accent'">{{ s.status_label ?? s.status }}</span>
            </div>
            <p class="mt-1 text-[14px] font-bold text-tutoring-text-hi">{{ s.topic ?? t('admin.bimbel.group_detail.session_default_title') }}</p>
          </div>
        </div>
      </template>

      <template v-else-if="tab === 'tasks'">
        <div v-if="activities.length === 0" class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-8 text-center text-sm text-tutoring-text-mid">
          {{ t('admin.bimbel.group_detail.empty_tasks') }}
        </div>
        <div v-else class="space-y-2">
          <div v-for="a in activities" :key="a.id" class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-3 flex items-center gap-3">
            <NavIcon :name="a.type === 'EXAM' ? 'check-circle' : a.type === 'QUIZ' ? 'sparkles' : 'book'" :size="20" class="text-tutoring-accent" />
            <div class="flex-1 min-w-0">
              <p class="text-[14px] font-bold text-tutoring-text-hi truncate">{{ a.title }}</p>
              <p class="text-[13px] text-tutoring-text-mid">{{ a.type_label ?? a.type }} · {{ t('admin.bimbel.group_detail.task_submissions', { count: a.submissions_count ?? 0 }) }}</p>
            </div>
          </div>
        </div>
      </template>
      ...
      <template v-else>
        <div class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-8 text-center text-sm text-tutoring-text-mid">
