...
<template>
  <div class="space-y-4 pb-12">
    ...
    <section class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-4 space-y-3">
      <h4 class="text-[15px] font-bold tracking-tight text-tutoring-text-hi">{{ t('admin.bimbel.appearance.mode_section') }}</h4>
      <div class="grid gap-2 sm:grid-cols-3">
        <button
          ...
          :class="theme.mode === m.id ? 'border-tutoring-accent bg-tutoring-accent-dim' : 'border-tutoring-border-soft hover:border-tutoring-border'"
        ...
        >
          ...
          />
          <p class="text-[14px] font-bold text-tutoring-text-hi">{{ m.label }}</p>
          <p class="text-[13px] text-tutoring-text-mid">{{ m.sub }}</p>
        </button>
      </div>
      <p v-if="theme.autoHint" class="text-[14px] text-tutoring-text-mid">{{ theme.autoHint }}</p>
    </section>

    <section class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-4">
      <h4 class="text-[15px] font-bold tracking-tight text-tutoring-text-hi mb-3">{{ t('admin.bimbel.appearance.language_section') }}</h4>
      <div class="flex gap-2">
        <button
          type="button"
          class="rounded-full border px-3 py-1.5 text-[14px] font-semibold"
          :class="locale === 'id' ? 'border-tutoring-accent bg-tutoring-accent-dim text-tutoring-accent' : 'border-tutoring-border bg-tutoring-panel text-tutoring-text-mid'"
          @click="locale = 'id'"
        >{{ t('admin.bimbel.appearance.language_id') }}</button>
        <button
          type="button"
          class="rounded-full border px-3 py-1.5 text-[14px] font-semibold"
          :class="locale === 'en' ? 'border-tutoring-accent bg-tutoring-accent-dim text-tutoring-accent' : 'border-tutoring-border bg-tutoring-panel text-tutoring-text-mid'"
          @click="locale = 'en'"
    ...
    <section class="rounded-2xl border border-tutoring-border-soft bg-tutoring-panel p-4">
      <h4 class="mb-2 text-[15px] font-bold tracking-tight text-tutoring-text-hi">{{ t('admin.bimbel.appearance.push_section') }}</h4>
      <div
        v-for="row in [
        ...
        :key="row.id"
        class="flex items-center justify-between border-t border-tutoring-border-soft py-2.5 first:border-t-0"
      >
        <div>
          <p class="text-[14px] font-bold text-tutoring-text-hi">{{ row.name }}</p>
          <p class="text-[14px] text-tutoring-text-mid">{{ row.sub }}</p>
        </div>
        <button
          type="button"
          class="h-5 w-9 rounded-full transition"
          :class="(notif as any)[row.id] ? 'bg-emerald-500' : 'bg-tutoring-border'"
          @click="toggle(row.id)"
        >
          <span
