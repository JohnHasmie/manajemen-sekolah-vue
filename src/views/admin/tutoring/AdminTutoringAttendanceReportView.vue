    ...
    <template v-else-if="data">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        ...
        <div class="rounded-2xl bg-bimbel-accent-dim p-3.5">
          <p class="text-[13px] font-bold uppercase tracking-widest text-bimbel-accent">{{ t('admin.bimbel.attendance_report.pill_excused') }}</p>
      ...
      <div class="grid gap-4 lg:grid-cols-5">
          ...
          <table class="w-full text-[14px]">
            ...
            <tbody>
              <tr v-if="groups.length === 0">
                <td colspan="4" class="px-3 py-6 text-center text-[14px] text-bimbel-text-mid">
              ...
              <tr v-for="r in groups" :key="r.group_id" class="border-t border-bimbel-border-soft">
                <td class="px-3 py-2.5">
                  ...
                  <p class="text-[13px] text-bimbel-text-mid">{{ t('admin.bimbel.attendance_report.students_count', { count: r.students_count }) }}</p>
                </td>
                <td class="px-3 py-2.5 text-bimbel-text-mid">{{ t('admin.bimbel.attendance_report.sessions_count', { count: r.sessions_count }) }}</td>
                <td class="px-3 py-2.5">
        ...
        <aside class="rounded-2xl border border-bimbel-border-soft bg-bimbel-panel p-4 lg:col-span-2 h-fit">
          <h4 class="text-[15px] font-bold tracking-tight text-bimbel-text-hi">{{ t('admin.bimbel.attendance_report.watchlist_title') }}</h4>
          <p class="text-[14px] text-bimbel-text-mid mb-3">{{ t('admin.bimbel.attendance_report.watchlist_subtitle') }}</p>
          <div v-if="watchlist.length === 0" class="text-[14px] text-bimbel-text-mid py-4 text-center">
          ...
          >
            ...
            <p class="text-[13px] text-bimbel-text-mid">{{ t('admin.bimbel.attendance_report.watchlist_row', { attended: w.attended, total: w.total, rate: w.attendance_rate, alpha: w.alpha_count }) }}</p>
          </div>
          <button
            v-if="watchlist.length > 0"
            type="button"
            class="mt-3 inline-flex items-center gap-1 rounded-lg border border-bimbel-border bg-bimbel-panel px-3 py-1.5 text-[14px] font-bold text-bimbel-text-hi hover:bg-bimbel-border-soft"
          >
