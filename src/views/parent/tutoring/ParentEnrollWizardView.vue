...
<template>
  <div class="space-y-4 pb-12">
    ...
    <div class="flex items-center gap-2 rounded-2xl border border-bimbel-border-soft bg-bimbel-panel p-3">
      <template v-for="s in [1, 2, 3, 4]" :key="s">
        <div class="flex items-center gap-2">
          ...
          <span class="hidden sm:inline text-[11.5px] font-semibold" :class="step === s ? 'text-bimbel-text-hi' : 'text-bimbel-text-mid'">
    ...
    <div v-if="successId" class="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-6 text-center">
      ...
      <button
        type="button"
        class="mt-4 rounded-lg bg-[#21afe6] px-4 py-2 text-[12.5px] font-bold text-white hover:opacity-90"
    ...
    <div v-else class="grid gap-4 lg:grid-cols-5">
      <div class="rounded-2xl border border-bimbel-border-soft bg-bimbel-panel p-4 lg:col-span-3 space-y-3">
        ...
        <div v-if="step === 1" class="space-y-3">
          <div>
            <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Pilih anak</p>
            <div class="mt-1 grid gap-2 sm:grid-cols-2">
              ...
              >
                <span class="grid h-8 w-8 place-items-center rounded-full bg-[#21afe6]/15 text-[#1a8fbe] dark:text-[#85d4f4] text-[11px] font-bold">
                  {{ c.name[0]?.toUpperCase() ?? '?' }}
                </span>
                <div class="min-w-0">
                  <p class="truncate text-[12.5px] font-bold text-bimbel-text-hi">{{ c.name }}</p>
                  <p class="truncate text-[10.5px] text-bimbel-text-mid">{{ c.class_name }}</p>
                </div>
              </button>
            </div>
          </div>
          <div>
            <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Pilih program</p>
            <select v-model="programId" class="mt-1 w-full rounded-lg border border-bimbel-border bg-bimbel-bg px-3 py-2 text-[12.5px] text-bimbel-text-hi focus:border-[#21afe6] focus:outline-none">
        ...
        <div v-else-if="step === 2" class="space-y-3">
          <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Paket tersedia</p>
          <div class="grid gap-2 sm:grid-cols-3">
            ...
            >
              ...
              <p class="text-[10.5px] text-bimbel-text-mid">{{ p.total_sessions ?? '–' }} sesi</p>
        ...
        <div v-else-if="step === 3" class="space-y-3">
          <div>
            <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Mode billing</p>
          ...
          <div v-if="groups.length">
            <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Pilih kelompok (opsional)</p>
            <select v-model="groupId" class="mt-1 w-full rounded-lg border border-bimbel-border bg-bimbel-bg px-3 py-2 text-[12.5px] text-bimbel-text-hi focus:border-[#21afe6] focus:outline-none">
          ...
          <div>
            <p class="text-[11px] font-bold uppercase tracking-wider text-bimbel-text-mid">Kode voucher</p>
            <div class="mt-1 flex gap-2">
              <input
                v-model="voucherCode"
                type="text"
                placeholder="Opsional"
                class="flex-1 rounded-lg border border-bimbel-border bg-bimbel-bg px-3 py-2 text-[12.5px] uppercase tracking-wider text-bimbel-text-hi focus:border-[#21afe6] focus:outline-none"
              />
              <button
                type="button"
                class="rounded-lg border border-bimbel-border bg-bimbel-panel px-3 py-2 text-[12.5px] font-bold text-bimbel-text-hi hover:bg-bimbel-border-soft"
                @click="tryVoucher"
              >Pakai</button>
            </div>
        ...
        <div v-else class="space-y-2">
          ...
          <div v-if="errorMsg" class="rounded-lg bg-rose-500/10 px-3 py-2 text-[11.5px] text-rose-700 dark:text-rose-300">{{ errorMsg }}</div>
        </div>
        ...
        <div class="flex gap-2 pt-3 border-t border-bimbel-border-soft">
          ...
          <button
            ...
            type="button"
            :disabled="!canNext"
            class="ml-auto rounded-lg bg-[#21afe6] px-4 py-2 text-[12.5px] font-bold text-white hover:opacity-90 disabled:opacity-50"
            @click="next"
          >Lanjut →</button>
          <button
            v-else
            type="button"
            :disabled="saving"
            class="ml-auto rounded-lg bg-emerald-600 px-4 py-2 text-[12.5px] font-bold text-white hover:opacity-90 disabled:opacity-50"
            @click="submit"
      ...
      <aside class="rounded-2xl border border-bimbel-border-soft bg-bimbel-panel p-4 lg:col-span-2 h-fit space-y-2">
        <h4 class="text-[12.5px] font-bold tracking-tight text-bimbel-text-hi">Ringkasan</h4>
