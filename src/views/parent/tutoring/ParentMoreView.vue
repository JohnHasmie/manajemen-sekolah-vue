<!--
  ...
  a tutoring-panel button with colored icon box, label, and sub. Routes
  ...
  <div class="space-y-3 pb-12">
    ...
    <template v-if="children.length > 0">
      ...
      <div class="grid gap-2" :class="children.length > 1 ? 'sm:grid-cols-2' : 'grid-cols-1'">
        <button
          ...
          :key="c.student_id"
          type="button"
          class="rounded-lg bg-tutoring-panel border border-tutoring-border-soft p-3 flex items-center gap-2.5 text-left transition-colors"
          :class="c.student_id === activeChildId ? 'border-tutoring-hero ring-1 ring-tutoring-hero/30' : 'hover:border-tutoring-border'"
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <p class="text-[14px] font-bold text-tutoring-text-hi truncate">{{ c.name }}</p>
            <p class="text-[12px] text-tutoring-text-mid truncate">{{ c.class_name || t('wali.bimbel.more.default_class_name') }}</p>
          </div>
          <span
    ...
    <div class="grid grid-cols-3 gap-2">
      <button
        ...
        :key="tile.label"
        type="button"
        class="rounded-md bg-tutoring-panel border border-tutoring-border-soft p-3.5 text-center"
        @click="go(tile.route)"
      >
    ...
    <div class="grid grid-cols-3 gap-2">
      <button
        ...
        @click="go(tile.route)"
      >
        <div class="w-[38px] h-[38px] rounded-lg mx-auto mb-1.5 grid place-items-center bg-tutoring-bg text-tutoring-text-hi">
          <NavIcon :name="tile.icon" :size="18" />
        </div>
        <p class="text-[13px] font-bold text-tutoring-text-hi">{{ tile.label }}</p>
        <p class="text-[10px] text-tutoring-text-mid mt-0.5">{{ tile.sub }}</p>
