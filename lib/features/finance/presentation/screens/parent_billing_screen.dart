...
class ParentBillingScreenState extends ConsumerState<ParentBillingScreen> {
  ...
  Future<void> _loadInitialData({bool useCache = true}) async {
    ...
    if (useCache) {
      ...
      final cachedStudents = await LocalCacheService.load(
        studentsCacheKey,
      ...
      if (cachedStudents != null &&
        ...
        if (!mounted) return;
        final parsedStudents = cachedStudents
  ...
  void _markBillReadIfNeeded() {
    if (_selectedStudentId == null) return;
    final hasUnread = _billingList.any(
      (b) => b['is_read'] != true && b['is_read'] != 1 && b['is_read'] != '1',
    );
    if (hasUnread) {
    ...
    try {
      ...
      if (source != null) {
        ...
        if (file != null && context.mounted) {
          ...
          final bool isValidFile = allowedExtensions.any(filePath.endsWith);

          if (!isValidFile) {
            if (context.mounted) {
              SnackBarUtils.showError(
                context,
                AppLocalizations.unsupportedFileFormat.tr,
              );
            }
            return;
          }
