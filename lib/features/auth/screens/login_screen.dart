...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _handleLogin() async {
    ...
    if (email.isEmpty || password.isEmpty) {
      SnackBarUtils.showInfo(context, AppLocalizations.emailPasswordNotEmpty.tr);
      return;
    }

    final response = await ref.read(authProvider.notifier).login(email, password);
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    try {
      ...
      final response = await ref.read(authProvider.notifier).googleLogin(
        email: account.email,
  ...
  void _showUnregisteredDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        icon: Icon(Icons.person_off_outlined, color: ColorUtils.darkBlue, size: 48),
        title: Text(
            ...
            return Card(
              ...
              child: ListTile(
                ...
                onTap: authState.isLoading ? null : () async {
                  final response = await ref.read(authProvider.notifier).selectSchool(sekolah['school_id'].toString());
                  _handleAuthResponse(response);
                },
              ),
            );
  ...
  Widget _buildRoleSelection(AuthState authState) {
    final schoolName = authState.selectedSchool?['school_name'] ?? authState.selectedSchool?['name'] ?? authState.selectedSchool?['nama_sekolah'] ?? authState.userData?['school_name'] ?? authState.userData?['nama_sekolah'] ?? '-';
        ...
        if (authState.schoolList.length > 1)
          TextButton(
            onPressed: () {
               ...
               ref.read(authProvider.notifier).resetToLogin(); // or provide a backToSchool() method
            },
            child: Text(AppLocalizations.backToLogin.tr), // For now go back to login instead of rewriting full flow backtracking
          )
        else
          TextButton(
            onPressed: () => ref.read(authProvider.notifier).resetToLogin(),
      ...
      children: [
          ...
          child: Column(
            children: [
              ...
              TextField(
                ...
                onSubmitted: (_) {
                  if (!authState.isLoading && emailController.text.trim().isNotEmpty && passwordController.text.isNotEmpty) {
                    _handleLogin();
                  }
                },
              ),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            onPressed: (authState.isServerConnected && !authState.isLoading) ? _handleLogin : null,
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed: (authState.isServerConnected && !authState.isLoading) ? _handleGoogleSignIn : null,
            icon: Image.asset(
            ...
            style: OutlinedButton.styleFrom(
              ...
              side: BorderSide(color: authState.isLoading ? Colors.grey : ColorUtils.darkBlue),
            ),
          ),
        ),
      ],
