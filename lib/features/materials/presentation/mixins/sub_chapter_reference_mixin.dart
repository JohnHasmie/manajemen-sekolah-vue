...
/// Mixin providing reference management for sub-chapter detail screen.
mixin SubChapterReferenceMixin on ConsumerState<SubBabDetailPage> {
  /// AI-generated data — bridge from state
  Map<String, dynamic>? get aiGeneratedData;
  set aiGeneratedData(Map<String, dynamic>? value);

  /// Regenerating references state — bridge from state
  bool get isRegeneratingRef;
  set isRegeneratingRef(bool value);

  /// Regenerate references.
  Future<void> regenerateReferences() async {
    final materialId = aiGeneratedData?['id']?.toString();
    if (materialId == null) return;

    setState(() => isRegeneratingRef = true);

    try {
      final response = await getIt<ApiSubjectService>().regenerateReferencesRaw(
        materialId,
      );
      if (!mounted) return;
      ...
      if (response.statusCode == 202) {
        ...
        if (jobId != null) {
          await _pollAndReload(
            ...
            jobId,
            onDone: () {
              if (mounted) setState(() => isRegeneratingRef = false);
            },
          );
          return;
        }
