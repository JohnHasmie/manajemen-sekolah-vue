...
void main() {
  ...
  testWidgets('TC007-TC011 - Admin Dashboard Tests', timeout: const Timeout(Duration(minutes: 5)), (tester) async {
    ...
    final adminMenuItems = [
      'Kelola Data', 'Kelola Jadwal', 'Pengumuman', 'Keuangan',
      'Laporan Presensi', 'Kelola RPP', 'Raport Siswa',
      'Pengaturan Sekolah', 'Kegiatan Kelas',
    ];
    ...
    for (final menu in menusToNavigate) {
      ...
      if (scrollable.evaluate().isNotEmpty) {
        for (final dir in [const Offset(0, -200), const Offset(0, 200)]) {
          try {
            await tester.dragUntilVisible(find.text(menu), scrollable.first, dir);
            break;
          } catch (_) {}
        }
        await tester.pumpAndSettle();
