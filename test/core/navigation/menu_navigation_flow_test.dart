...
Widget _buildMenuGrid(List<({String title, dynamic icon, String destination})> items) {
  return MaterialApp(
    home: Scaffold(
...
void main() {
  group('MenuItemCard — navigation flow: tap → screen → back', () {
    testWidgets('tapping "Data Siswa" opens student screen and back returns to menu',
        (tester) async {
      await tester.pumpWidget(_buildMenuGrid([
        (title: 'Data Siswa', icon: Icons.person_outline, destination: 'Student Screen'),
      ]));
    ...
    testWidgets('each menu item navigates to its own unique screen', (tester) async {
      await tester.pumpWidget(_buildMenuGrid([
        (title: 'Jadwal', icon: Icons.calendar_today, destination: 'Schedule Screen'),
        (title: 'Keuangan', icon: Icons.payments, destination: 'Finance Screen'),
        (title: 'Pengumuman', icon: Icons.campaign, destination: 'Announcement Screen'),
      ]));
    ...
    testWidgets('back button always restores correct menu title', (tester) async {
      await tester.pumpWidget(_buildMenuGrid([
        (title: 'RPP', icon: Icons.article, destination: 'Lesson Plan Screen'),
      ]));
  ...
  group('MenuItemCard — deep navigation (3 levels)', () {
    testWidgets('level1 → level2 → level3 → back → back → menu', (tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Builder(builder: (ctx1) {
          return Scaffold(
            appBar: AppBar(title: const Text('Menu')),
            body: MenuItemCard(
              title: 'Nilai',
              icon: Icons.grade,
              onTap: () => AppNavigator.push(ctx1,
  ...
  group('QuickActionButton — navigation flow: tap → screen → back', () {
    testWidgets('tapping quick action opens screen and back returns', (tester) async {
      await tester.pumpWidget(_buildQuickActions([
        (label: 'Absensi', icon: Icons.fact_check, destination: 'Attendance Screen'),
      ]));
  ...
  group('Mixed navigation — back-stack integrity', () {
    testWidgets('navigating between different menus preserves correct back stack',
        (tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Builder(builder: (ctx0) {
          return Scaffold(
            ...
            body: Column(
              children: [
                MenuItemCard(
                  title: 'Section A',
                  icon: Icons.folder,
                  onTap: () => AppNavigator.push(ctx0,
                    Builder(builder: (ctxA) {
                      return Scaffold(
                        appBar: AppBar(title: const Text('Section A')),
                        body: MenuItemCard(
                          title: 'Item A1',
                          icon: Icons.article,
                          onTap: () => AppNavigator.push(ctxA,
                            Scaffold(
                              appBar: AppBar(title: const Text('Page A1')),
    ...
    testWidgets('pushReplacement on logout — clears back stack', (tester) async {
      await tester.pumpWidget(MaterialApp(
            ...
            body: Column(
              children: [
                ElevatedButton(
                  ...
                  onPressed: () => AppNavigator.push(ctx,
                    Scaffold(
                ...
                ElevatedButton(
                  ...
                  onPressed: () => AppNavigator.pushAndClearStack(ctx,
