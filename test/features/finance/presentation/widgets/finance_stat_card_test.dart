...
void main() {
  group('FinanceStatCard', () {
    ...
    testWidgets('label text is clamped to a single line via maxLines:1', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap(testCard));
    ...
    testWidgets(
      'widget tree contains a Column (icon on top, value, label beneath)',
      (tester) async {
        await tester.pumpWidget(_wrap(testCard));
    ...
    testWidgets('label overflow is ellipsis', (tester) async {
      ...
      final labelText = tester.widget<Text>(
        find.text('Belum Dibayar Bulan Ini'),
      );
    ...
    testWidgets('icon container is 40x40', (tester) async {
      ...
      final iconContainer = containers.firstWhere(
        (c) => c.constraints?.minWidth == 40 || (c.child is Icon),
