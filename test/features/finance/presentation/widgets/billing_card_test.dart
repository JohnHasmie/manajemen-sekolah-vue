...
void main() {
  ...
  group('BillingCard', () {
    ...
    testWidgets('displays billing description', (tester) async {
      await tester.pumpWidget(_build(
        billing: _makeBilling(description: 'Tagihan bulan April'),
      ));
    ...
    testWidgets('shows "Terverifikasi" status badge for verified (Indonesian)',
        (tester) async {
      await tester.pumpWidget(_build(billing: _makeBilling(status: 'verified')));
    ...
    testWidgets('shows "Unpaid" status badge for unpaid (English)', (tester) async {
    ...
    testWidgets('shows "Verified" status badge for verified (English)', (tester) async {
      final lp = LanguageProvider()..setLanguage(LanguageProvider.english);
      await tester.pumpWidget(_build(
    ...
    testWidgets('shows "Pending" status badge for pending (English)', (tester) async {
      final lp = LanguageProvider()..setLanguage(LanguageProvider.english);
      await tester.pumpWidget(_build(
    ...
    testWidgets('shows "-" for name when all name keys are null', (tester) async {
      await tester.pumpWidget(_build(billing: {
        'status': 'unpaid',
        'amount': 100000,
        'is_read': false,
      }));
    ...
    testWidgets('is_read as string "1" renders without crashing', (tester) async {
      await tester.pumpWidget(_build(billing: _makeBilling(isRead: '1')));
      expect(find.byType(BillingCard), findsOneWidget);
    });
    ...
    testWidgets('renders long billing name with overflow handling', (tester) async {
      await tester.pumpWidget(_build(
        billing: _makeBilling(
          name: 'Pembayaran SPP dan Dana Kegiatan Siswa Semester Genap Tahun Ajaran 2024/2025',
        ),
      ));
      expect(find.byType(BillingCard), findsOneWidget);
    });
