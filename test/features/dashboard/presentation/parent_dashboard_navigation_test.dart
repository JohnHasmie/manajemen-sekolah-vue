          ...
          child: Column(
            ...
            children: [
                ...
                QuickActionButton(
                  ...
                  onTap: () => AppNavigator.push(ctx,
                      _subScreen(title: 'Announcements Screen', children: [
                        (label: 'Pengumuman PPDB', dest: 'PPDB Announcement Detail'),
                        (label: 'Libur Nasional', dest: 'Holiday Announcement Detail'),
                      ])),
                ),
                QuickActionButton(
                  label: 'Billing',
                  ...
                  color: primaryColor,
                  onTap: () => AppNavigator.push(ctx,
                      _subScreen(title: 'Parent Billing Screen', children: [
                        (label: 'SPP Bulan Ini', dest: 'Monthly SPP Detail'),
                        (label: 'Riwayat Pembayaran', dest: 'Payment History View'),
                      ])),
                ),
              ]),
              ...
              MenuItemCard(
                ...
                onTap: () => AppNavigator.push(ctx,
                    _subScreen(title: 'Class Activities Parent Screen', children: [
                      (label: 'Kegiatan Hari Ini', dest: 'Today Activity Detail'),
                      (label: 'Kegiatan Kemarin', dest: 'Yesterday Activity Detail'),
                    ])),
              ),
              MenuItemCard(
                title: 'Grades',
                ...
                onTap: () => AppNavigator.push(ctx,
                    _subScreen(title: 'Parent Grades Screen', children: [
                      (label: 'Matematika', dest: 'Math Grade Detail'),
                      (label: 'Bahasa Indonesia', dest: 'Bahasa Grade Detail'),
                                ...
                                body: ListTile(
                                  ...
                                  onTap: () => AppNavigator.push(
                                    ...
                                    Scaffold(
                                      ...
                                      body: const Text('Attendance detail data'),
                                    ),
                                  ),
                                ),
                              );
              ...
              MenuItemCard(
                ...
                onTap: () => AppNavigator.push(ctx,
                    _subScreen(title: 'Parent Report Card Screen', children: [
...
void main() {
  ...
  group('Parent Dashboard — Quick Actions navigation', () {
    testWidgets('Announcements (quick) → Announcements Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapQuick(tester, 'Announcements');
      await _assertScreenAndBack(tester, 'Announcements Screen', 'Parent Dashboard');
    });

    testWidgets('Billing (quick) → Parent Billing Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapQuick(tester, 'Billing');
      await _assertScreenAndBack(tester, 'Parent Billing Screen', 'Parent Dashboard');
    });
  });
  ...
  group('Parent Dashboard — Menu Items single-level navigation', () {
    ...
    testWidgets('Class Activities → Class Activities Parent Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapMenu(tester, 'Class Activities');
      await _assertScreenAndBack(
          tester, 'Class Activities Parent Screen', 'Parent Dashboard');
    });

    testWidgets('Grades → Parent Grades Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapMenu(tester, 'Grades');
      await _assertScreenAndBack(tester, 'Parent Grades Screen', 'Parent Dashboard');
    });

    testWidgets('Presence → Select Student Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapMenu(tester, 'Presence');
      await _assertScreenAndBack(tester, 'Select Student Screen', 'Parent Dashboard');
    });

    testWidgets('Billing (menu) → Parent Billing Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapMenu(tester, 'Billing');
      await _assertScreenAndBack(tester, 'Parent Billing Screen', 'Parent Dashboard');
    });

    testWidgets('eReport Card → Parent Report Card Screen → back', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
      await _tapMenu(tester, 'eReport Card');
      await _assertScreenAndBack(tester, 'Parent Report Card Screen', 'Parent Dashboard');
    });
  });
  ...
  group('Parent Dashboard — Deep: Announcements', () {
    testWidgets('Dashboard → Announcements → PPDB Detail → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
    ...
    testWidgets('Dashboard → Announcements → Holiday Detail → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Deep: Class Activities', () {
    testWidgets('Dashboard → Class Activities → Today → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
    ...
    testWidgets('Dashboard → Class Activities → Yesterday → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Deep: Grades', () {
    ...
    testWidgets('Dashboard → Grades → Bahasa Indonesia → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
    ...
    testWidgets('Dashboard → Grades → IPA → back → back → Dashboard', (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Deep: Presence', () {
    testWidgets('Dashboard → Presence → Ahmad → Attendance → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Deep: Billing', () {
    testWidgets('Dashboard → Billing → SPP Bulan Ini → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
    ...
    testWidgets('Dashboard → Billing → Riwayat Pembayaran → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Deep: eReport Card', () {
    testWidgets('Dashboard → eReport Card → Semester 1 → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
    ...
    testWidgets('Dashboard → eReport Card → Semester 2 → back → back → Dashboard',
        (tester) async {
      await tester.pumpWidget(_buildParentDashboard());
  ...
  group('Parent Dashboard — Back-stack isolation', () {
    ...
    testWidgets('all 6 menu items cycle without stack pollution', (tester) async {
      final menus = [
