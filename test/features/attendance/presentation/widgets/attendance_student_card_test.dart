// Tests for AttendanceStudentCard — a student row with optional editing buttons.
...
void main() {
  group('AttendanceStudentCard', () {
    ...
    testWidgets('displays student name', (tester) async {
      await tester.pumpWidget(_build(student: _makeStudent(name: 'Siti Rahayu')));
      expect(find.text('Siti Rahayu'), findsOneWidget);
    });

    testWidgets('displays NIS with prefix', (tester) async {
      await tester.pumpWidget(_build(student: _makeStudent(studentNumber: '20240042')));
    ...
    testWidgets('tapping H button calls onStatusChanged with "hadir"',
        (tester) async {
      String? changed;
      await tester.pumpWidget(_build(isEditing: true, onStatusChanged: (s) => changed = s));
    ...
    testWidgets('different index values render without crashing', (tester) async {
    ...
    testWidgets('long student name is truncated with ellipsis', (tester) async {
      await tester.pumpWidget(_build(
        student: _makeStudent(name: 'Muhammad Abdullah Bin Hassan Al-Rashidi'),
      ));
