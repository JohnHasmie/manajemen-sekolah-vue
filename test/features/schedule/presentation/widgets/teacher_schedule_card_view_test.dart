...
void main() {
  ...
  group('isHourCurrent', () {
    ...
    test('returns false when now is after end time', () {
      final schedule = {'jam_mulai': '01:00:00', 'jam_selesai': '02:00:00'};
    ...
    test('returns false when now is before start time', () {
      final schedule = {'jam_mulai': '23:50:00', 'jam_selesai': '23:59:00'};
      final now = DateTime.now();
  ...
  group('Scroll target finding', () {
    ...
    test(
      'targets first lesson of next future day when today has no schedules',
      () {
        final now = DateTime.now();
  ...
  group('Day header today detection', () {
    ...
    test('tomorrow is not past and not today', () {
      ...
      if (now.weekday < 7) {
        final tomorrowName =
            dayOrder[now.weekday]; // weekday is 1-indexed, array is 0-indexed
