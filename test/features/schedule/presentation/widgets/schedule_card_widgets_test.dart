...
void main() {
  group('ScheduleFilterSectionHeader', () {
    ...
    testWidgets('renders without crashing for long title', (tester) async {
      ...
      expect(
        find.text('Filter Berdasarkan Hari dan Jam Pelajaran'),
        findsOneWidget,
      );
    });

    testWidgets('does not render onPressed — purely decorative', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
  ...
  group('ScheduleInfoTag', () {
    ...
    testWidgets('text has maxLines 1 and ellipsis overflow', (tester) async {
      ...
      final textWidget = tester.widget<Text>(
        find.text('Nama kelas yang sangat panjang'),
      );
  ...
  group('ScheduleCircleActionButton', () {
    ...
    testWidgets('different callback instances fire independently', (
      tester,
    ) async {
      int editCount = 0;
      int deleteCount = 0;
